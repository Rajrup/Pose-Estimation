...
class PoseRecognition:
  ...
  def Apply(self, batched_data_dict, batch_size, istub, multiperson_tracker, my_lock):
    if (batch_size != len(batched_data_dict["humans"])):
  ...
  def GetBatchedResultArray(self, batched_result_dict, batch_size):
    if (batch_size != len(batched_result_dict["output_label"])):
      ...
      for i in range(batch_size):
        result_dict = dict()
        result_dict["output_label"] = batched_result_dict["output_label"][i]
